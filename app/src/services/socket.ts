...
class SocketService {
  ...
  private groupMessageEditListeners: Listener[] = [];
  private groupMessageDeleteListeners: Listener[] = [];
  private groupReactionListeners: Listener[] = [];
  private groupTypingListeners: Listener[] = [];
  private callMuteListeners: Listener<{ userId: string; isMuted: boolean }>[] =
    [];
  private callScreenShareListeners: Listener<{
    userId: string;
    isSharing: boolean;
  }>[] = [];
  ...
  connect(token: string): void {
    ...
    this.socket.on("call:video-upgrade:request", (data) => {
      this.videoUpgradeListeners.forEach((l) =>
        l({ ...data, type: "request" }),
      );
    });
    ...
    this.socket.on("webrtc:ice-candidate", (data) => {
      this.webrtcListeners.forEach((l) =>
        l({ ...data, webrtcType: "ice-candidate" }),
      );
    });
    ...
    this.socket.on("group:message:edit", (data) => {
      this.groupMessageEditListeners.forEach((l) => l(data));
    });

    this.socket.on("group:message:delete", (data) => {
      this.groupMessageDeleteListeners.forEach((l) => l(data));
    });

    this.socket.on("group:message:reaction", (data) => {
      this.groupReactionListeners.forEach((l) => l(data));
    });
  ...
  // Emit message edit
  emitMessageEdit(messageId: string, content: string): Promise<void> {
    return new Promise((resolve, reject) => {
      if (!this.socket) return reject(new Error("Socket not connected"));
      this.socket.emit("message:edit", { messageId, content }, (res: any) => {
        if (res.success) resolve();
  ...
  // Emit message delete
  emitMessageDelete(messageId: string): Promise<void> {
    return new Promise((resolve, reject) => {
      if (!this.socket) return reject(new Error("Socket not connected"));
      this.socket.emit("message:delete", { messageId }, (res: any) => {
        if (res.success) resolve();
  ...
  offTyping(l: Listener) {
    this.typingListeners = this.typingListeners.filter((x) => x !== l);
  }

  onStatus(l: Listener<{ userId: string; status: string }>) {
    this.statusListeners.push(l);
  }
  offStatus(l: Listener<{ userId: string; status: string }>) {
    this.statusListeners = this.statusListeners.filter((x) => x !== l);
  }

  onNotification(l: Listener) {
    this.notificationListeners.push(l);
  }
  offNotification(l: Listener) {
  ...
  onCall(l: Listener) {
    this.callListeners.push(l);
  }
  offCall(l: Listener) {
    this.callListeners = this.callListeners.filter((x) => x !== l);
  }

  onWebrtc(l: Listener) {
    this.webrtcListeners.push(l);
  }
  offWebrtc(l: Listener) {
    this.webrtcListeners = this.webrtcListeners.filter((x) => x !== l);
  }
  onWebRTC(l: Listener) {
    this.webrtcListeners.push(l);
  }
  ...
  onReadReceipt(l: Listener) {
    this.readReceiptListeners.push(l);
  }
  offReadReceipt(l: Listener) {
    this.readReceiptListeners = this.readReceiptListeners.filter(
      (x) => x !== l,
    );
  }

  onReaction(l: Listener) {
    this.reactionListeners.push(l);
  }
  offReaction(l: Listener) {
    this.reactionListeners = this.reactionListeners.filter((x) => x !== l);
  }

  onMessageEdit(l: Listener) {
    this.messageEditListeners.push(l);
  }
  offMessageEdit(l: Listener) {
    this.messageEditListeners = this.messageEditListeners.filter(
      (x) => x !== l,
    );
  }

  onMessageDelete(l: Listener) {
    this.messageDeleteListeners.push(l);
  }
  offMessageDelete(l: Listener) {
    this.messageDeleteListeners = this.messageDeleteListeners.filter(
      (x) => x !== l,
    );
  }

  onFriend(l: Listener) {
    this.friendListeners.push(l);
  }
  offFriend(l: Listener) {
    this.friendListeners = this.friendListeners.filter((x) => x !== l);
  }

  onCallMuteToggle(l: Listener<{ userId: string; isMuted: boolean }>) {
    this.callMuteListeners.push(l);
  }
  offCallMuteToggle(l: Listener<{ userId: string; isMuted: boolean }>) {
    this.callMuteListeners = this.callMuteListeners.filter((x) => x !== l);
  }

  onCallScreenShareToggle(l: Listener<{ userId: string; isSharing: boolean }>) {
    this.callScreenShareListeners.push(l);
  }
  offCallScreenShareToggle(
    l: Listener<{ userId: string; isSharing: boolean }>,
  ) {
    this.callScreenShareListeners = this.callScreenShareListeners.filter(
      (x) => x !== l,
    );
  }

  onVideoUpgrade(l: Listener) {
    this.videoUpgradeListeners.push(l);
  }
  offVideoUpgrade(l: Listener) {
    this.videoUpgradeListeners = this.videoUpgradeListeners.filter(
      (x) => x !== l,
    );
  }
  ...
  sendGroupMessage(
    ...
    return new Promise((resolve, reject) => {
      ...
      this.socket.emit(
        "group:message:send",
        { groupId, content, type, duration, replyToId },
        (res: any) => {
  ...
  startGroupTyping(groupId: string) {
    this.socket?.emit("group:typing:start", { groupId });
  }
  stopGroupTyping(groupId: string) {
    this.socket?.emit("group:typing:stop", { groupId });
  }

  // Group message actions
  emitGroupMessageEdit(
    groupId: string,
    messageId: string,
    content: string,
  ): void {
    this.socket?.emit("group:message:edit", { groupId, messageId, content });
  }

  emitGroupMessageDelete(groupId: string, messageId: string): void {
    this.socket?.emit("group:message:delete", { groupId, messageId });
  }
  ...
  emitGroupReaction(
    ...
    this.socket?.emit("group:message:reaction", {
      groupId,
      messageId,
      emoji,
      action,
  ...
  // Listener registration for new group events
  onGroupMessageEdit(l: Listener) {
    this.groupMessageEditListeners.push(l);
  }
  offGroupMessageEdit(l: Listener) {
    this.groupMessageEditListeners = this.groupMessageEditListeners.filter(
      (x) => x !== l,
    );
  }

  onGroupMessageDelete(l: Listener) {
    this.groupMessageDeleteListeners.push(l);
  }
  offGroupMessageDelete(l: Listener) {
    this.groupMessageDeleteListeners = this.groupMessageDeleteListeners.filter(
      (x) => x !== l,
    );
  }

  onGroupReaction(l: Listener) {
    this.groupReactionListeners.push(l);
  }
  offGroupReaction(l: Listener) {
    this.groupReactionListeners = this.groupReactionListeners.filter(
      (x) => x !== l,
    );
  }

  onGroupMessage(l: Listener) {
    this.groupMessageListeners.push(l);
  }
  offGroupMessage(l: Listener) {
    this.groupMessageListeners = this.groupMessageListeners.filter(
      (x) => x !== l,
    );
  }

  onGroupTyping(l: Listener) {
    this.groupTypingListeners.push(l);
  }
  offGroupTyping(l: Listener) {
