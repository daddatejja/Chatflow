...
export const createPoll = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { messageId, question, options, isMultiple, isAnonymous, endsAt } =
      req.body;

    if (!messageId || !question || !options || !Array.isArray(options)) {
      return res
        .status(400)
        .json({ error: "messageId, question and options are required" });
    }
...
export const votePoll = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!pollId || !optionIds || !Array.isArray(optionIds)) {
      return res
        .status(400)
        .json({ error: "pollId and optionIds array are required" });
    }
    ...
    if (!poll.isMultiple && optionIds.length > 1) {
      return res
        .status(400)
        .json({ error: "This poll does not allow multiple votes" });
    }
...
export const getPollByMessageId = async (
  ...
  try {
    ...
    console.error("Error fetching poll by messageId:", error);
    res.status(500).json({ error: "Failed to fetch poll" });
  }
};
